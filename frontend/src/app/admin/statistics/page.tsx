...
export default function StatisticsPage() {
  ...
  const metrics = [
    { label: "Tổng lượt chuyển đổi", value: total.toLocaleString("vi-VN"), note: "30 ngày gần nhất", icon: TrendingUp, color: "text-[#0b4dcc]" },
    ...
    <section>
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((item, index) => (
          <article key={item.label} className={`${adminPanelClass} min-h-[112px] p-5 ${index === 1 ? "border-l-4 border-l-[#0b4dcc]" : ""}`}>
      ...
      <article className={`${adminPanelClass} mt-6 p-5`}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><h2 className="text-xl font-bold">Số lượt chuyển đổi theo ngày</h2><p className="mt-1 text-sm text-[#586075]">Dữ liệu tổng hợp từ 30 ngày gần nhất</p></div>
          <span className="rounded-lg bg-[#0b4dcc] px-4 py-2 text-sm font-semibold text-white">30 ngày qua</span>
        </div>
        <div className="mt-5 h-[300px]">
          {isLoading ? <div className="h-full animate-pulse rounded-xl bg-slate-50" /> : points.length ? (
            <svg viewBox={`-15 -10 ${width + 30} ${height + 35}`} className="h-full w-full" preserveAspectRatio="none">
              <defs><linearGradient id="statisticsArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0b4dcc" stopOpacity=".2" /><stop offset="100%" stopColor="#0b4dcc" stopOpacity="0" /></linearGradient></defs>
              {[0, 1, 2, 3, 4].map((item) => <line key={item} x1="0" x2={width} y1={item * 78} y2={item * 78} stroke="#e1e5ef" />)}
              <path d={area} fill="url(#statisticsArea)" />
              <path d={line} fill="none" stroke="#0b4dcc" strokeWidth="3" vectorEffect="non-scaling-stroke" />
              {points.map(({ x, y, item }) => <circle key={item.statDate} cx={x} cy={y} r="3.5" fill="#fff" stroke="#0b4dcc" strokeWidth="2" vectorEffect="non-scaling-stroke"><title>{`${item.statDate}: ${item.totalConversions}`}</title></circle>)}
            </svg>
      ...
      <article className={`${adminPanelClass} mt-6 overflow-hidden`}>
        <div className="flex flex-col gap-3 border-b border-[#d8dceb] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-lg font-bold">Thống kê theo công cụ</h2>
          <label className="flex h-10 items-center gap-3 rounded-lg border border-[#bcc5df] bg-[#eef1ff] px-3 sm:w-64"><Search size={18} /><input value={search} onChange={(event) => { setSearch(event.target.value); setToolPage(1); }} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Tìm kiếm công cụ..." /></label>
        </div>
          ...
          <table className="w-full min-w-[850px] text-left">
            ...
            <tbody>
              {visibleTools.map((item) => {
                ...
                return (
                  <tr key={item.tool} className="border-t border-[#dfe3ee] text-sm">
        ...
        {toolPages > 1 && (
          <div className="flex items-center justify-between border-t border-[#d8dceb] px-5 py-3.5 text-sm text-slate-500">
