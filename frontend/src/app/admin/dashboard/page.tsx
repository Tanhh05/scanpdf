          ...
          <article
            ...
            className={`min-h-[142px] rounded-lg border p-4 shadow-[0_2px_6px_rgba(30,41,59,0.035)] ${
              card.tone === "solid"
                ...
                : "border-[#e2e8f0] bg-white text-[#0f172a] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
            }`}
          >
            <div className="flex items-start justify-between">
              <span className={`flex h-10 w-10 items-center justify-center rounded-lg ${
                card.tone === "solid" ? "bg-white/15" : card.tone === "green" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" : card.tone === "blue" ? "bg-[#eff6ff] text-[#2563eb] dark:bg-[#2563eb]/15 dark:text-sky-300" : "bg-slate-100 text-slate-600 dark:bg-slate-950 dark:text-slate-300"
              }`}>
                <card.icon size={20} />
              </span>
              <span className={`text-xs font-semibold ${card.tone === "solid" ? "text-blue-100" : card.tone === "green" ? "text-emerald-700" : "text-[#2563eb]"}`}>
                {card.note}
              </span>
            </div>
            <p className={`mt-4 text-[11px] font-semibold uppercase tracking-wide ${card.tone === "solid" ? "text-blue-100" : "text-[#475569] dark:text-slate-300"}`}>{card.label}</p>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,2.1fr)_minmax(290px,0.9fr)]">
        <article className={`${adminPanelClass} p-5`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748b]">Lượt chuyển đổi PDF trong 7 ngày qua</p>
            </div>
            <span className="rounded-lg border border-[#cbd5e1] bg-[#f8fafc] px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">7 ngày gần nhất</span>
          </div>
          <div className="mt-5 h-[260px]">
            {statistics.isLoading ? (
              ...
              <svg viewBox={`-10 -10 ${width + 20} ${height + 45}`} className="h-full w-full" preserveAspectRatio="none" aria-label="Biểu đồ chuyển đổi">
                ...
                <path d={line} fill="none" stroke="#2563eb" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
                {points.map(({ x, y, item }) => (
                  <circle key={item.statDate} cx={x} cy={y} r="4" fill="#fff" stroke="#2563eb" strokeWidth="2" vectorEffect="non-scaling-stroke">
        ...
        <article className={`${adminPanelClass} overflow-hidden`}>
          ...
          <div className="space-y-5 p-5">
            {[
              { label: "Dung lượng ổ đĩa", value: 45, icon: Server, color: "bg-[#2563eb]" },
              { label: "CPU Usage", value: 12, icon: Cpu, color: "bg-slate-600" },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-4">
                <span className="flex h-12 w-12 items-center justify-center rounded-lg bg-[#eff6ff] text-[#2563eb] dark:bg-[#2563eb]/15 dark:text-sky-300"><item.icon size={22} /></span>
            ...
            <div className="border-t border-[#e2e8f0] pt-5 dark:border-slate-800">
              ...
              <div className="mt-5 space-y-5 text-sm">
                ...
                <div><p className="flex items-center gap-3"><i className="h-2.5 w-2.5 rounded-full bg-[#2563eb]" /> Tỷ lệ thành công: {data?.successRate ?? 0}%</p><span className="ml-5 mt-1 block text-xs text-slate-500">Toàn hệ thống</span></div>
              </div>
            </div>
          </div>
      ...
      <article className={`${adminPanelClass} mt-6 overflow-hidden`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left">
            <thead className="bg-[#eff6ff] text-xs uppercase tracking-wide text-[#475569] dark:bg-slate-800 dark:text-slate-200">
            ...
            <tbody>
              {recentUsers.data?.items.map((user) => (
                <tr key={user.id} className="border-t border-[#e3e6ef] text-sm dark:border-slate-800 dark:text-slate-100">
                  ...
                  <td className="px-5 py-4 font-medium text-[#2563eb]">{user.subscriptions[0]?.plan.name ?? "Free"}</td>
                  <td className="px-5 py-4 text-center">
                    <button
                      type="button"
                      onClick={() => setSelectedUser(user)}
                      className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition hover:bg-[#eff6ff] hover:text-[#2563eb] dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-sky-300"
                      title="Xem nhanh"
                    >
      ...
      {selectedUser && (
        <article className={`${adminPanelClass} mt-5 p-5`}>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#cbd5e1] px-4 text-sm font-semibold text-[#2563eb] hover:bg-[#eff6ff] dark:border-slate-700 dark:text-sky-300 dark:hover:bg-slate-800"
              >
                <Eye size={17} />
                Sao chép email
              </button>
          ...
          <div className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
            <div className="rounded-lg bg-[#f8fafc] p-4 dark:bg-slate-800">
            ...
            <div className="rounded-lg bg-[#f8fafc] p-4 dark:bg-slate-800">
              ...
              <p className="mt-2 font-semibold text-[#2563eb]">{selectedUser.subscriptions[0]?.plan.name ?? "Free"}</p>
            </div>
          </div>
        </article>
      )}
