  ...
  return (
    <section>
      <AdminPageHeader
        ...
        action={
          <button type="button" onClick={() => setStatus((value) => value ? "" : "ACTIVE")} className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#cbd5e1] bg-white px-4 text-sm font-semibold dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
            <Filter size={19} /> {status ? "Bỏ lọc" : "Lọc dữ liệu"}
          </button>
        }
      />

      <div className="mt-5 flex max-w-sm items-center gap-3 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] px-4 dark:border-slate-700 dark:bg-slate-900">
      ...
      <article className={`${adminPanelClass} mt-5 overflow-hidden`}>
          ...
          <table className="w-full min-w-[1000px] text-left">
            ...
            <tbody>
              {users.data?.items.map((user, index) => (
                <tr key={user.id} className={`border-t border-[#e2e8f0] text-sm dark:border-slate-800 ${user.status === "SUSPENDED" ? "bg-slate-50/70 text-slate-500 dark:bg-slate-800/55 dark:text-slate-400" : "dark:text-slate-100"}`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <AdminAvatar name={user.fullName} tone={user.status === "SUSPENDED" ? "slate" : index % 3 === 0 ? "blue" : "slate"} />
                      <div><strong className={`block text-[15px] ${user.status === "SUSPENDED" ? "line-through text-slate-400" : "text-[#0f172a] dark:text-slate-50"}`}>{user.fullName}</strong><span className="text-xs text-slate-500 dark:text-slate-400">{user.email}</span></div>
                    </div>
                  </td>
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[#e2e8f0] px-5 py-3.5 text-sm dark:border-slate-800 sm:flex-row">
