  ...
  return (
    <section>
      <AdminPageHeader
        ...
        action={
          <button type="button" onClick={() => setStatus((value) => value ? "" : "ACTIVE")} className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#aeb8d3] bg-white px-4 text-sm font-semibold">
            <Filter size={19} /> {status ? "Bỏ lọc" : "Lọc dữ liệu"}
          </button>
        }
      />

      <div className="mt-5 flex max-w-sm items-center gap-3 rounded-full border border-[#bcc5df] bg-[#eef1ff] px-4">
      ...
      {message && <p className="mt-4 rounded-xl bg-[#edf2ff] px-4 py-3 text-sm font-semibold text-[#0b4dcc]">{message}</p>}
          ...
          <table className="w-full min-w-[1000px] text-left">
            ...
            <tbody>
              {users.data?.items.map((user, index) => (
                <tr key={user.id} className={`border-t border-[#dfe3ee] text-sm ${user.status === "SUSPENDED" ? "bg-slate-50/70 text-slate-500" : ""}`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <AdminAvatar name={user.fullName} tone={user.status === "SUSPENDED" ? "slate" : index % 3 === 0 ? "blue" : "slate"} />
                      <div><strong className={`block text-[15px] ${user.status === "SUSPENDED" ? "line-through" : "text-[#111527]"}`}>{user.fullName}</strong><span className="text-xs text-slate-500">{user.email}</span></div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                    <select
                      ...
                      className="max-w-40 rounded-lg border-0 bg-transparent py-2 font-medium outline-none"
                    >
                  ...
                  <td className="px-4 py-3.5"><span className="rounded-md bg-[#dfe6fb] px-2.5 py-1 text-xs font-semibold">{user.role}</span></td>
