...
export default function PaymentsPage() {
  ...
  const metrics = [
    { label: "Tổng doanh thu", value: `${revenue.toLocaleString("vi-VN")}đ`, note: "Trong trang hiện tại", icon: CircleDollarSign, color: "text-[#10aee8]" },
  ...
  return (
    <section>
      ...
      <article className={`${adminPanelClass} mt-6 overflow-hidden`}>
        <div className="flex flex-col gap-3 border-b border-[#d8ded5] dark:border-slate-800 p-4 xl:flex-row xl:items-center">
          <label className="flex h-10 min-w-0 flex-1 items-center gap-3 rounded-lg border border-[#b8c8be] bg-[#eef8fd] px-3 xl:max-w-sm">
            <Search size={21} />
          ...
          <button type="button" onClick={exportCsv} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#10aee8] px-4 text-sm font-semibold text-white shadow-md">
            ...
            <tbody>
              {visibleItems.map((item) => (
                <tr key={item.id} className="border-t border-[#d8ded5] dark:border-slate-800 text-sm">
                  <td className="px-5 py-3.5 font-medium text-[#10aee8]">#{item.transactionCode}</td>
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[#d8ded5] px-5 py-3.5 text-sm sm:flex-row">
      ...
      {selectedPayment && (
        <article className={`${adminPanelClass} mt-5 p-5`}>
          ...
          <div className="mt-4 grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-lg bg-[#f8faf7] p-4 dark:bg-slate-900"><p className="text-slate-500">Người dùng</p><p className="mt-2 font-semibold">{selectedPayment.user.fullName}</p><p className="text-slate-500">{selectedPayment.user.email}</p></div>
            <div className="rounded-lg bg-[#f8faf7] p-4 dark:bg-slate-900"><p className="text-slate-500">Gói</p><p className="mt-2 font-semibold">{selectedPayment.plan.name}</p></div>
            <div className="rounded-lg bg-[#f8faf7] p-4 dark:bg-slate-900"><p className="text-slate-500">Số tiền</p><p className="mt-2 font-semibold">{selectedPayment.amount.toLocaleString("vi-VN")}đ</p></div>
            <div className="rounded-lg bg-[#f8faf7] p-4 dark:bg-slate-900"><p className="text-slate-500">Trạng thái</p><div className="mt-2"><AdminStatusBadge status={selectedPayment.status} /></div><p className="mt-2 text-slate-500">{new Date(selectedPayment.createdAt).toLocaleString("vi-VN")}</p></div>
          </div>
        </article>
      )}
    </section>
