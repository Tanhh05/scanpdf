  ...
  return (
    <AccountLayout>
      <div className="space-y-7">
        <div>
          <p className="text-sm font-bold text-indigo-600">TỔNG QUAN TÀI KHOẢN</p>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600"><Gauge size={21} /></span>
            <p className="mt-5 text-sm text-slate-500">Lượt còn lại hôm nay</p>
            <p className="mt-1 text-3xl font-black text-slate-950">{profile.data?.remainingToday ?? "-"}</p>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600"><CheckCircle2 size={21} /></span>
            <p className="mt-5 text-sm text-slate-500">Gói hiện tại</p>
            <p className="mt-1 text-3xl font-black text-slate-950">{profile.data?.plan.name ?? "-"}</p>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-amber-600"><Clock3 size={21} /></span>
            <p className="mt-5 text-sm text-slate-500">Đã dùng hôm nay</p>
            <p className="mt-1 text-3xl font-black text-slate-950">{profile.data?.usedToday ?? "-"}</p>
          </article>
        </div>
        ...
        <article className="rounded-2xl bg-slate-950 p-6 text-white sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-300">Gói hiện tại</p>
          ...
          <div className="mt-8">
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-indigo-400 transition-all" style={{ width: `${usagePercent}%` }} />
            </div>
          </div>
          ...
          <Link href="/pricing" className="mt-8 inline-flex rounded-xl bg-white px-5 py-3 font-black text-slate-950 hover:bg-indigo-50">
            Xem các gói dịch vụ
          </Link>
        </article>
      </div>
