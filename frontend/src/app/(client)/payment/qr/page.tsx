  ...
  if (payment.data?.status === "PAID") {
    return (
      <div className="card max-w-lg p-10 text-center">
        ...
        <h1 className="app-heading mt-5 text-3xl">Đã kích hoạt gói {payment.data.plan.name}</h1>
  ...
  if (payment.data?.status === "CANCELLED" || payment.data?.status === "FAILED") {
    return (
      <div className="card max-w-lg p-10 text-center">
        ...
        <h1 className="app-heading mt-5 text-3xl">Thanh toán không còn hiệu lực</h1>
  ...
  return (
    <div className="card w-full max-w-4xl p-8">
      <div className="text-center">
        <h1 className="app-heading text-3xl">Quét VietQR để nâng cấp {payment.data?.plan.name ?? ""}</h1>
        <p className="mt-2 text-slate-500">Mở ứng dụng ngân hàng và quét mã bên dưới.</p>
      </div>
      <div className="mt-8 grid items-center gap-8 md:grid-cols-2">
        <div className="flex min-h-80 items-center justify-center rounded-lg border border-[#d8ded5] bg-[#f8faf7] p-4 dark:border-slate-800 dark:bg-slate-950">
          {qrImage
        ...
        <dl className="space-y-4 text-sm">
          ...
          <div><dt className="text-slate-500">Số tiền</dt><dd className="mt-1 text-2xl font-black text-[#10aee8]">{payment.data?.amount.toLocaleString("vi-VN") ?? "..."}đ</dd></div>
          ...
          <p className="rounded-lg bg-amber-50 p-3 text-amber-800">Giữ nguyên số tiền và nội dung để hệ thống tự động xác nhận.</p>
        </dl>
      </div>
...
function CopyButton({ value }: { value?: string }) {
  return (
    <button type="button" title="Sao chép" className="cursor-pointer text-slate-400 hover:text-[#10aee8]" onClick={() => value && navigator.clipboard.writeText(value)}>
      <Copy size={15} />
    </button>
  );
}
