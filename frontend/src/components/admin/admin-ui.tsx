...
export const adminPanelClass =
  "rounded-xl border border-[#d8dceb] bg-white shadow-[0_2px_6px_rgba(30,41,59,0.035)]";

export const adminInputClass =
  "h-10 w-full rounded-lg border border-[#bcc5df] bg-white px-3 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#0b4dcc] focus:ring-2 focus:ring-[#0b4dcc]/10";
...
export function AdminPageHeader({
  ...
  return (
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div>
        {title && <h2 className="text-[22px] font-bold tracking-[-0.02em] text-[#111527]">{title}</h2>}
        {description && <p className={`${title ? "mt-1" : ""} text-sm text-[#535b70]`}>{description}</p>}
      </div>
      {action}
    </div>
  );
